...
class MeteoDataset(Dataset):
    ...
    def __init__(self, rain_dir, input_length, output_length, temporal_stride, dataset, recurrent_nn=False,
                 wind_dir=None):
        ...
        if dataset == 'valid':
            self.rain_files_names = [val for (idx, val) in enumerate(self.rain_files_names) if
                                     filter_one_week_over_two_for_eval(idx) == 0]

        if dataset == 'test':
            self.rain_files_names = [val for (idx, val) in enumerate(self.rain_files_names) if
                                     filter_one_week_over_two_for_eval(idx) == 1]
        ...
        if wind_dir != None:
            self.rain_files_names, self.U_wind_files_names, self.V_wind_files_names = keep_wind_when_rainmap_exists(
                self.rain_files_names,
                self.U_wind_files_names,
                self.V_wind_files_names)

        self.indices = []
        for i in range(
                (len(self.rain_files_names) - self.input_length - self.output_length) // self.temporal_stride + 1):
            sequence = self.rain_files_names[
    ...
    def __getitem__(self, i):
        ...
        rain_files_names_i = self.rain_files_names[
                             self.temporal_stride * idx: self.temporal_stride * idx + self.input_length + self.output_length]
        if self.wind_dir != None:
            U_wind_files_names_i = self.U_wind_files_names[
                                   self.temporal_stride * idx: self.temporal_stride * idx + self.input_length + self.output_length]
            V_wind_files_names_i = self.V_wind_files_names[
                                   self.temporal_stride * idx: self.temporal_stride * idx + self.input_length + self.output_length]
